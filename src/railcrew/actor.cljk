(ns railcrew.actor
  "RailCrewActor — the ISCO-08 8311 locomotive engine drivers actor as
  a `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one rail-crew scheduling/logistics
  ...
  so an interrupted run can resume after human sign-off. This actor
  ...
  The unconditional invariant: the RailCrewAdvisor can never directly
  commit a record the RailCrewGovernor refuses — every commit-record!
  call is gated behind `:decide`, and no commit path can ever finalize
  a locomotive-movement/throttle/brake-control or signal-override/
  stop-signal-departure decision (see `railcrew.governor`'s closed
  op-allowlist and scope-exclusion rule, both of which are checked
  independently of the advisor's own restraint)."
...
(defn build-graph
  "Build a compiled RailCrewActor graph. `store` implements
  `railcrew.store/Store`. `advisor` implements
  `railcrew.advisor/Advisor` (defaults to `mock-advisor`).
